...
const ProductCard = ({ product }) => {
  return (
    <div className="border rounded-md p-4 shadow-md bg-slate-100 hover:shadow-lg hover:scale-105 transition-transform duration-300 ease-in-out">
      <img
        src={`http://localhost:5000/uploads/${product.image}`} // Concatenate the base URL with image filename
        alt={product.name}
        className="w-3/4 h-40 object-cover rounded-md mx-auto"
      />
      <h2 className="text-lg font-semibold mt-4 text-gray-800 hover:text-blue-600 transition-colors cursor-default">
        {product.name}
      </h2>
      <p className="text-gray-600 mt-2 cursor-default">${product.price}</p>
      <Link
